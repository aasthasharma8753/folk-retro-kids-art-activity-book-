...
| **Resolve the Panel 4/8 character block in `docs/SCROLL_NARRATIVE.md`** — a proposed "Mowgli & Bagheera" panel is **not cleared** and should not proceed as drafted | Kipling's original *Jungle Book* text is public domain, but "Mowgli" and "Bagheera" as visually recognized today are inseparable from Disney's actively-trademarked 1967 film designs — a real infringement/trade-dress risk, categorically worse than the Doordarshan-serial-design caution above, and inconsistent with the roster's Indian-mythology sourcing anyway |
...
- **Selected engineering path — "permanent structural inaccessibility"**: the magnet is never a loose or extractable part, full stop, rather than relying on a pull-force floor to keep an accessible magnet in place. Candidate construction to bring to the lab as a starting hypothesis: magnets flush-seated into routed pockets in the greyboard cover plate, sealed with epoxy, then capped with a rigid shield layer laminated over the pocket and wrapped under the cover's fabric/canvas finish — i.e., a child would have to destroy the cover structure itself to reach a magnet. Exact material grades, magnet count/size, and pocket dimensions are open engineering choices for the Phase 1 prototype, not settled by this roadmap.
...
2. One full accordion-scroll per character, in the order Ghatotkach → Betaal → Tenali Raman → Bal Hanuman. A full 10-panel draft layout for the Ghatotkach scroll is mapped in `docs/SCROLL_NARRATIVE.md` — **do not send Panel 4/8 to illustration until the character block above is resolved.**
